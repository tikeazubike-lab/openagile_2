...
**Owner**: Infrastructure / Antigravity (via Alembic migrations)
...
**Owner**: Jointly defined here — Antigravity implements, Lovable consumes
...
**Owner**: Lovable.dev (frontend) + Antigravity (integration wiring)
...
This is the definitive endpoint list Lovable uses to build API hooks. Antigravity implements these exactly.
...
BETA BANNER (demo subdomain only):
  ...
  Background: #1a1a2e, text: "BETA — testdrive.epm.zubbystudio.shop"
...
## Part D: Implementation Order for Antigravity
...
8. Traefik: `testdrive.epm.zubbystudio.shop` Traefik labels (old Streamlit container unchanged)
...
| Beta subdomain | testdrive.epm.zubbystudio.shop parallel to old app | LOCKED |
...
**Antigravity receives**: Parts A, B, D, E (full brief)
**Next action**: Antigravity begins Phase 2A. Lovable begins Login + Dashboard + Holdings pages in parallel.
**Sync point**: After Phase 2A Foundation — Antigravity shares base API URL + running auth endpoint with Lovable for integration testing.
