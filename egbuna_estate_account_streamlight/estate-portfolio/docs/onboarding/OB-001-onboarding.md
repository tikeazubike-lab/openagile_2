...
The openagile repository root already has an `AGENTS.md` covering all 5
projects. Do NOT modify or replace it.

The new files go **inside the EPM project subdirectory**:
`egbuna_estate_account_streamlight/estate-portfolio/`

Antigravity creates these files **locally** in that subdirectory
...
```bash
# Working directory: egbuna_estate_account_streamlight/estate-portfolio/
mkdir -p .cursor/rules

# Write the following files from the Claude output documents:
#   AGENTS.md                        → estate-portfolio/ (EPM-specific, not root)
...
git commit -m "chore: add Cursor rules and AGENTS.md for EPM agent onboarding"
...
**Expected answer**: No — push to test branch. GitHub Actions will build
...
Implement all three. Do not run anything locally. Push to test when done
and write a handover brief to Claude.
```

**Success criteria**:
- Cursor produces code changes for all three files
- Cursor suggests pushing to test (not running locally)
...
Push to test when complete. Write handover to Claude.
```
...
4. Cursor merges to test if clean, fixes issues if not

**For debugging:**
1. Push to test → check GitHub Actions logs in your browser first
2. If the issue is on the server (container crash, DB error, runtime exception):
   ...
   Read logs: `docker compose logs epm_v3 --tail=100`
