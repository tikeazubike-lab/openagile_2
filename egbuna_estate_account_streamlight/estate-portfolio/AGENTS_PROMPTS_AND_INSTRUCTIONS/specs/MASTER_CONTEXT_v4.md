...
Local Development:
  ...
  Editors: Neovim, Cursor, Deepseek:flash
```
...
ALWAYS (Local Workstation):
  ...
  - Only: Edit files in Neovim, Cursor, or Deepseek:flash
...
```
Deepseek:flash (execute) → Claude (review + document) → Done
```

**Zone 2 Tasks** (Architecture):
```
Claude (design + ToT) → Grok (verify + current info) → Deepseek:flash (implement)
```

**UI Generation**:
```
Claude (wireframe + Lovable prompt) → Lovable (React code → GitHub PR)
→ Deepseek:flash (review PR, strip Supabase, wire build, deploy)
```

**Testing**:
```
Claude (STLC spec) → Codex (migrate + execute on test branch, SSH to VPS)
→ Codex handover → Claude (review drift) → Deepseek:flash (fix backend gaps)
```

**Debugging**:
```
Deepseek:flash (diagnose via SSH to VPS) → Claude (root cause analysis)
→ Grok (check if known issue) → Deepseek:flash (fix + push)
```
...
**Tool Strengths**:
- **Deepseek:flash**: File system access, multi-file edits, FastAPI/Docker/GitHub Actions
...
**Missing backend modules** (Deepseek:flash must build — Phase 2B):
